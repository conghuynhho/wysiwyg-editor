...
export default class Undo {
  ...
  constructor({ editor, config = {}, onUpdate, maxLength }) {
    ...
    const { debounceTimer = defaultOptions.config.debounceTimer } = config
   ...
   *
   * @param {Array} stack  Changes history stack.
  ...
  save(state) {
    ...
    const caretIndex =
      state[indexInState] && (
        ...
        : null
    console.log({ index: indexInState, state: cloneDeep(state), caretIndex })
    this.stack.push({ index: indexInState, state: cloneDeep(state), caretIndex })
    this.position += 1
    this.onUpdate()
  }
  ...
  getCaretIndex(index) {
    ...
    const caretBlock = new VanillaCaret(blocks[index].firstChild || blocks[index])
    ...
    if (this.canRedo()) {
      ...
      if (block) {
        if(state[index].type === 'image') {
          this.editor.render({blocks: state})
        } else {
          this.blocks.update(block.id, state[index].data)
          this.setCaretIndex(index, caretIndex)
        }
  ...
  setEventListeners() {
    ...
    const pressedKeys = (e, keys, compKeys) => {
      ...
      return !!verifyListThreeKeysPressed(e, keys)
    }
